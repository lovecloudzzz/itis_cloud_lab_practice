...
def delete(album: str, path: str) -> None:
    """
    A method to delete photos from the given album and path.
    :param album: Album name in the bucket.
    :param path: Path to the photos in the local directory.
    :return: None
    """
    # Checking if the path exists and it is a directory
    if not os.path.exists(path) or not os.path.isdir(path):
    ...
    for picture in pictures:
        # Deleting photos by their full path
